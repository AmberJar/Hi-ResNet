...
import random
import os
from utils.losses import AdMSoftmaxLoss
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        adm_softmax_loss_func = AdMSoftmaxLoss()
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            adm_softmax_loss = adm_softmax_loss_func(output, target)
            adm_softmax_loss_aux = adm_softmax_loss_func(out_aux, target)
            ...
            if isinstance(self.loss, torch.nn.DataParallel):
                ...
                # loss1 = 0.4 * loss + 1 * se_connectivity_loss + .4 * (0.4 * loss_aux + 1 * se_connectivity_loss_aux)

            # loss1 = loss + .4 * loss_aux + .1 * (se_connectivity_loss + .4 * se_connectivity_loss_aux)
            loss1 = loss + .4 * loss_aux + (adm_softmax_loss + .4 * adm_softmax_loss_aux)
            ...
            tbar.set_description(
                'TRAIN ({}) | Loss: {:.3f} | Acc {:.2f} mIoU {:.2f} | B {:.2f} D {:.2f} | {} | {:.3f} |'.format(
                    ...
                    pixAcc, mIoU,
                    self.batch_time.average, self.data_time.average, Class_IoU, loss.item()))
