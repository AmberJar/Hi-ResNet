    ...
    def forward(self, x):
        ...
        for i in range(H_out):
            ...
            for j in range(W_out):
                hs = int(torch.tensor(i * H_in / H_out).floor())
                he = int(torch.tensor((i + 1) * H_in / H_out).ceil())

                ws = int(torch.tensor(j * W_in / W_out).floor())
                we = int(torch.tensor((j + 1) * W_in / W_out).ceil())
...
class PSPModule(nn.Module):
    ...
    def _make_stages(self, in_channels, out_channels, bin_sz):
        ...
        # prior = CostomAdaptiveAvgPool2D(output_size=(bin_sz, bin_sz), input_size=(32, 32))
    ...
    def forward(self, features):
        ...
        # print('pyramids.size: ', type(pyramids))
...
class FPN_fuse(nn.Module):
    ...
    def forward(self, features):
        print(len(self.conv1x1))
        features[1:] = [conv1x1(feature) for feature, conv1x1 in zip(features[1:], self.conv1x1)]
        # for i, (feature, conv1x1) in enumerate(zip(features[1:], self.conv1x1)):
        #     features[i+1] = conv1x1(feature)
        # print('============================')
        # for i, conv1x1 in enumerate(self.conv1x1):
        #     features[i+1] = conv1x1(features[i+1])
        #
        # for i in range(len(features)-1):
        #     features[i+1] = self.conv1x1[i](features[i+1])

        P = [up_and_add(features[i], features[i-1]) for i in reversed(range(1, len(features)))]
        # P = []
        # for i in reversed(range(1, len(features))):
        #     P.append(up_and_add(features[i], features[i-1]))
