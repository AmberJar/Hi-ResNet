...
from models.hrnet_configV2 import MODEL_CONFIGS
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels,
        ...
        if stride != 1 or \
            ...
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(num_features=num_channels[branch_index] * block.expansion,
                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            nn.BatchNorm2d(num_features=num_inchannels[i], momentum=bn_momentum),
                        )
                    )
                elif j == i:
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    nn.BatchNorm2d(num_features=num_outchannels_conv3x3, momentum=bn_momentum)
                                )
                            )
                        else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, bn_type, bn_momentum, **kwargs):
        ...
        if os.environ.get('full_res_stem'):
            ...
            stem_stride = 1
            self.conv1 = nn.Conv2d(4, 64, kernel_size=3, stride=stem_stride, padding=1,
                                   bias=False) # 輸入通道數3->4
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            nn.BatchNorm2d(num_features=num_channels_cur_layer[i], momentum=bn_momentum),
                            nn.ReLU(inplace=False)
                        )
                    )
                else:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            ...
                            nn.BatchNorm2d(num_features=outchannels, momentum=bn_momentum),
                            nn.ReLU(inplace=False)
                        )
                    )
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, bn_type=None, bn_momentum=0.1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(num_features=planes * block.expansion, momentum=bn_momentum)
            )
