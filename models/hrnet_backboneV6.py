...
from models.hrnet_config_V6 import MODEL_CONFIGS
...
class CA(nn.Module):
    def __init__(self, inp, reduction=32, momentum=0.1):
        self.channels = 640
        ...
        # self.bn1 = nn.BatchNorm2d(mip, momentum)
        self.act = nn.GELU()
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, bn_type=None, bn_momentum=0.1, drop_path=0.):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False, groups=inplanes)
        ...
        self.ca = CA(planes, momentum=bn_momentum)
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True, bn_type=None, bn_momentum=0.1, drop_paths=None):
        ...
        self.branches = self._make_branches(
            num_branches, blocks, num_blocks, num_channels, bn_type=bn_type, bn_momentum=bn_momentum, drop_paths=drop_paths)
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels,
                         stride=1, bn_type=None, bn_momentum=0.1, drop_paths=None):
    ...
    def _make_branches(self, num_branches, block, num_blocks, num_channels, bn_type, bn_momentum=0.1, drop_paths=None):
        branches = []

        for i in range(num_branches):
            branches.append(
                self._make_one_branch(i, block, num_blocks, num_channels, bn_type=bn_type, bn_momentum=bn_momentum, drop_paths=drop_paths)
            )
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, bn_type, bn_momentum, drop_path_rate=0.5, **kwargs):
        ...
        # stochastic depth
        depth_s1 = cfg["STAGE1"]["NUM_BLOCKS"][0]
        ...
        if os.environ.get('full_res_stem'):
            ...
            self.relu = nn.GELU()
            self.layer1 = self._make_layer(Bottleneck, 64, 64, 4, drop_path=dp_rates[0:depth_s1], bn_type=bn_type, bn_momentum=bn_momentum)
        else:
            stem_stride = 2
            self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=stem_stride, padding=1,
                                   bias=False)
        ...
        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels, drop_path=dp_rates[depth_s1: depth_s1 + depth_s2], bn_type=bn_type, bn_momentum=bn_momentum)
        ...
        self.stage3, pre_stage_channels = self._make_stage(
            self.stage3_cfg, num_channels, drop_path=dp_rates[depth_s1 + depth_s2: depth_s1 + depth_s2 + depth_s3], bn_type=bn_type, bn_momentum=bn_momentum)
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, drop_path=dp_rates[depth_s1 + depth_s2 + depth_s3:depth_s1 + depth_s2 + depth_s3 + depth_s4], multi_scale_output=True, bn_type=bn_type, bn_momentum=bn_momentum)

        if os.environ.get('keep_imagenet_head'):
            print('keep_imagenet_head, yes')
            self.incre_modules, self.downsamp_modules, \
            self.final_layer = self._make_head(pre_stage_channels, drop_path=dp_rates, bn_type=bn_type, bn_momentum=bn_momentum)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, drop_path=None, bn_type=None, bn_momentum=0.1):
        downsample = None
        ...
        layers = []
        layers.append(block(inplanes, planes * block.expansion, stride, downsample, bn_type=bn_type, drop_path=drop_path[0], bn_momentum=bn_momentum))

        inplanes = planes * block.expansion
        planes = planes * block.expansion

        for i in range(1, blocks):
            cur_drop_path = drop_path[i]
            layers.append(block(inplanes, planes, drop_path=cur_drop_path, bn_type=bn_type, bn_momentum=bn_momentum))

        return nn.Sequential(*layers)

    def _make_stage(self, layer_config, num_inchannels,
                    multi_scale_output=True, drop_path=None, bn_type=None, bn_momentum=0.1):
        ...
        for i in range(num_modules):
            ...
            modules.append(
                HighResolutionModule(
                    ...
                    bn_type,
                    bn_momentum,
                    drop_paths=drop_path[num_blocks[0] * i: num_blocks[0] * (i + 1)],
                )
            )
...
def HRNetBackbone(arch):
    if arch == 'hrnet18':
        ...
        arch_net = HighResolutionNet(MODEL_CONFIGS['hrnet64'],
                                     bn_type=None,
                                     bn_momentum=0.1)
    elif arch == 'hrnet48_large':
        arch_net = HighResolutionNet(MODEL_CONFIGS['hrnet48_large'],
                                     bn_type=None,
                                     bn_momentum=0.1)
    else:
...
if __name__ == '__main__':
    ...
    x_in = torch.rand(size=(2, 3, 256, 256))
    outs = model(x_in)
