...
from utils.detail_loss import DetailAggregateLoss
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            boundery_bce_loss, boundery_dice_loss = boundary_loss_func(edge, target)
            ...
            if self.config['arch']['type'][:3] == 'PSP':
                ...
                loss2 = self.loss(output[1], target) * 0.4 + loss
                output = output[0]
            else:
                assert output.size()[2:] == target.size()[1:]
                assert output.size()[1] == self.num_classes
                loss2 = self.loss(output, target)

            if isinstance(self.loss, torch.nn.DataParallel):
                loss2 = loss2.mean()
                # boundery_bce_loss = boundery_bce_loss.mean()
                # boundery_dice_loss = boundery_dice_loss.mean()
                hd_loss = hd_loss.mean()
            # if epoch % 3 == 0:
            #     loss1 = loss2 + .05 * boundery_bce_loss + .1 * boundery_dice_loss + .8 * hd_loss
            # else:
            loss1 = loss2 + .8 * hd_loss
            # loss1 = loss2 + .05*boundery_bce_loss + .1*boundery_dice_loss + .8*hd_loss
            ...
            tbar.set_description(
                'TRAIN ({}) | Loss: {:.3f} | Acc {:.2f} mIoU {:.2f} | B {:.2f} D {:.2f} | {} | {:.3f} |'.format(
                    ...
                    pixAcc, mIoU,
                    self.batch_time.average, self.data_time.average, Class_IoU, loss1.item()))
