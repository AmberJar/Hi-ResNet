...
class HuaWeiDataset(BaseDataSet):
    ...
    def _set_files(self):
        ...
        self.offset_dir = os.path.join(self.root, self.split, "offset")
    ...
    def _load_data(self, index):
        ...
        offset_path = os.path.join(self.offset_dir, offset_id + '.mat')
        ...
        if self.segfix:
            distance_map, angle_map = self._load_maps(offset_path, label)
            return image, label, image_id, distance_map, angle_map
        else:
...
class Huawei(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None,
        ...
        self.MEAN = [0.338543, 0.336788, 0.362383]
        self.STD = [0.103899, 0.106971, 0.103965]
        ...
        super(Huawei, self).__init__(self.dataset, batch_size, num_workers=num_workers, val_split=val_split, shuffle=shuffle, sampler=self.train_sampler)
