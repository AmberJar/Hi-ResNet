...
class Road(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None,
        ...
        self.dataset = RoadDataset(**kwargs)
        self.train_sampler = DistributedSampler(self.dataset)

        super(Road, self).__init__(self.dataset, batch_size, num_workers=num_workers, val_split=val_split, shuffle=shuffle, sampler=self.train_sampler)
