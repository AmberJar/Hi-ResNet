...
class Trainer(BaseTrainer):
    def __init__(self, model, loss, resume, config, train_loader=None, val_loader=None, local_rank=None, train_logger=None, prefetch=True):
        ...
        if self.device == torch.device('cpu'): prefetch = True
        if prefetch:
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            # tbar.sampler.set_epoch(epoch) # test
            ...
            assert output.size()[2:] == target.size()[1:]   #C,N,H,W :n,h,w
            ...
            if self.local_rank == 0:
                if batch_idx % self.log_step == 0:
                    ...
                    self.writer.add_scalar(f'{self.wrt_mode}/loss', loss1.item(), self.wrt_step)

            # FOR EVAL
            seg_metrics = eval_metrics(output, target, self.num_classes)
            self._update_seg_metrics(*seg_metrics)
            pixAcc, mIoU, Class_IoU = self._get_seg_metrics(self.num_classes).values()
        ...
        seg_metrics = self._get_seg_metrics(self.num_classes)
        if self.local_rank == 0:
            for k, v in list(seg_metrics.items())[:-1]:
                self.writer.add_scalar(f'{self.wrt_mode}/{k}', v, self.wrt_step)
            for i, opt_group in enumerate(self.optimizer.param_groups):
                self.writer.add_scalar(f'{self.wrt_mode}/Learning_rate_{i}', opt_group['lr'], self.wrt_step)
                # self.writer.add_sc alar(f'{self.wrt_mode}/Momentum_{k}', opt_group['momentum'], self.wrt_step)
    ...
    def _valid_epoch(self, epoch):
        self.val_num_classes = self.val_loader.dataset.num_classes
        ...
        if self.local_rank == 0:
            tbar = tqdm(self.val_loader, ncols=140)
        else:
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):  # [0,1,0,1]
                ...
                # PRINT INFO
                pixAcc, mIoU, _ = self._get_seg_metrics(self.num_classes).values()
                if self.local_rank == 0:
                    tbar.set_description(
            ...
            palette = self.palette.AdaSpaceMaps_palette
    ...
    def _get_seg_metrics(self, num_classes):
