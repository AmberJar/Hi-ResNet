...
def mask2source(image_path, mask_path, output_path, alpha):
    ...
    print(np.unique(label))
    # label[label != 0] = 1
    pd[np.where(label == 0)] = [0, 0, 0]  #
    pd[np.where(label == 1)] = [0,255,255] # 工业区 橙色
    # pd[np.where(label == 2)] = [0,0,255] # 居民区 红色
    # pd[np.where(label == 3)] = [0,255,255] #浅黄色
    # pd[np.where(label == 4)] = [0,255,0] # 浅绿
    # pd[np.where(label == 5)] = [0, 200, 250] #深黄色
    # pd[np.where(label == 6)] = [0,255,127]  # 深绿
    pd[np.where(label == 7)] = [0,0,255]  # 紫色
...
def main():
    ...
    with torch.no_grad():
        ...
        for img_file in tbar:
            file_name = img_file.split('/')[-1].split('.')[0] + args.extension
            image = Image.open(img_file).convert('RGB')
            image_ = to_tensor(image)
            print(image_.shape)

            mask = Image.open(os.path.join(args.mask, file_name)).convert('L')
            mask_ = to_tensor(mask)

            input_ = torch.cat([image_, mask_], dim=0)
            print(input_.size())
            ...
            if args.mode == 'multiscale':
                ...
                prediction = sliding_predict(model, input, num_classes, window_size=1024)
            else:
...
def parse_arguments():
    ...
    parser.add_argument('-c', '--config', default='config_hrnet_val.json', type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='sliding', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    # '/data/fpc/saved/HRNET_HDLOSS/road_second_0.3/best_model.pth'
    #  '/data/fpc/saved/HRNET_HDLOSS/10-19_10-04/best_model.pth'
    # '/data/fpc/saved/HRNET_VAL/10-27_18-14/checkpoint-epoch27.pth'
    # /data/fpc/saved/HRNET_VAL/10-28_18-33
    parser.add_argument('-m', '--model', default='/data/fpc/saved/HRNET_VAL/10-28_18-33/best_model.pth', type=str,
                        help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-i', '--images', default=r'/data/fpc/inference/taian_data/taian_splited_new_6png', type=str,
                        help='Path to the images to be segmented')
    parser.add_argument('-o', '--output', default='/data/fpc/output/outputs_12_14_val_01', type=str,
                        help='Output Path')
    parser.add_argument('-e', '--extension', default='.png', type=str,
                        help='The extension of the images to be segmented')
    parser.add_argument('-p', '--probabilities', default=True, type=bool,
                        help='generate probabilities prediction')
    # parser.add_argument('-suf', '--suffix', default='.png', type=str,
    #                     help='output image type')

    # /data/fpc/output/outputs_10_28/probabilities_mask_output
    parser.add_argument('-ma', '--mask', default='/data/fpc/output/outputs_12_14_roads_01', type=str,
                        help='input mask address')
