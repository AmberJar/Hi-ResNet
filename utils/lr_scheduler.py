...
class Learning_Rate_Finder(_LRScheduler):
    def __init__(self, optimizer, num_epochs, iters_per_epoch=0,
                 warmup_epochs=0, last_epoch=-1):
        self.iters_per_epoch = iters_per_epoch
        self.cur_iter = 0
        self.N = num_epochs * iters_per_epoch
        self.warmup_iters = warmup_epochs * iters_per_epoch
        self.lr_mult = self.lr_mult = (1 / 1e-5) ** (1 / 100)
        self.factor = 1
        super(Learning_Rate_Finder, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        T = self.last_epoch * self.iters_per_epoch + self.cur_iter
        self.factor = self.factor * self.lr_mult
        if self.warmup_iters > 0 and T < self.warmup_iters:
            factor = 1.0 * T / self.warmup_iters
...
class OneCycle(_LRScheduler):
    def __init__(self, optimizer, num_epochs, iters_per_epoch=0, last_epoch=-1,
        ...
        self.low_lrs = [opt_grp['lr'] / div_factor for opt_grp in
                        optimizer.param_groups]
        self.final_lrs = [opt_grp['lr'] / (div_factor * 1e4) for opt_grp in
                          optimizer.param_groups]
    ...
    def get_lr(self):
        ...
        if T <= self.phase1_iters:
            ...
            for i in range(len(self.optimizer.param_groups)):
                self.optimizer.param_groups[i]['momentum'] = self.momentums[
                                                                 0] + self.mom_diff * cos_anneling
